...
# http://api.metagenomics.anl.gov/1/project?verbosity=full&limit=10&offset=0
...
jsonobject = GET_url(base_url, "dump_project_metadata.py")
...
for i in range(0, total_count / limit +1):
    parameters = {"limit": limit, "offset": i * limit, "verbosity" :"full"}
    base_url = API_URL + CALL + "?" + urllib.urlencode(parameters)
    jsonstructure = json.loads(GET_url(base_url))

    klist = printall(jsonstructure, ("metadata",), klist) 
